...
class DashboardOrder(BaseModel):
    ...
    valor: float
    anotacao_tecnica: Optional[str]
    endereco: Optional[str]
    referencia: Optional[str] = "N/A"

app = FastAPI(title="Cbnet Support System Unified")

# Monta arquivos estáticos
app.mount("/static", StaticFiles(directory="src/v3/static"), name="static")
...
async def serve_v1_maps():
    with open("src/v3/static/index.html", "r", encoding="utf-8") as f:
        return f.read()
...
async def get_filtered_support_orders(tipo_filtro: Optional[str] = None, data_filtro: Optional[str] = None, id_funcionario_filtro: Optional[int] = None,
    ...
    for item in raw_data:
        ...
        structured_orders.append(DashboardOrder(
            ...
            anotacao_tecnica=item.get("anotacao_tecnica", "N/A"),
            endereco=item.get("endereco", "N/A")
        ))
    return structured_orders

@app.get("/api/client/{cli_id}/referencia")
async def get_client_referencia(cli_id: int):
    try:
        client = await fetch_client_data(str(cli_id))
        return {"referencia": client.get("referencia", "N/A")}
    except Exception:
        return {"referencia": "N/A"}
...
async def serve_dashboard():
    with open("src/v3/index.html", "r", encoding="utf-8") as f:
        return f.read()
